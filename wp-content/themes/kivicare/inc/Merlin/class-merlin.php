...
class Merlin
{
	...
	public string $ready_big_button_url;
	public string $slug;
	...
	function __construct($config = array(), $strings = array())
	{
		...
		$this->base_path              = $config['base_path'] . "/inc";
		$this->base_url               = $config['base_url'] . "/inc";
		...
		add_action('init', array($this, 'kivicare_download_file'));
	}
		...
		<body class="merlin__body merlin__body--<?php echo esc_attr($current_step); ?>">
			...
			<div class="wizard-progress current-step-<?php echo esc_attr($this->step); ?>">
				<div class="step <?php echo esc_attr($class_welcome); ?>">
					<?php echo esc_html__("Welcome", "kivicare"); ?>
					<div class="node">
	...
	public function loading_spinner()
	{
		...
		$spinner = 'inc/' . $this->directory . '/assets/images/spinner';
		...
		<div class="merlin__content--transition">
			...
			<div class="content-right">
				<div class="content-right-inner">
					...
					<table>
						<tr class="heading">
							<th><?php esc_html_e('PHP Configuration', 'kivicare') ?></th>
							<th><?php esc_html_e('Recommended (Min)', 'kivicare') ?></th>
							<th><?php esc_html_e('Existing', 'kivicare') ?></th>
						</tr>
						<tr>
						...
						<tr>
							...
							<td><code><?php echo ini_get('max_input_vars'); ?></code></td>
						</tr>
						<tr>
							<td>max_input_time</td>
							<td><code>300</code></td>
							<td><code><?php echo ini_get('max_input_time'); ?></code></td>
						</tr>
						<tr>
							<td>memory_limit</td>
		...
		<div class="merlin__content--transition">
			...
			<div class="content-right">
				<div class="content-right-inner">
					...
					<p>
						<strong><?php esc_html_e("About child theme", "kivicare"); ?></strong><br>
						<?php
						esc_html_e("A child theme allows you to change small aspects of your site's appearance yet still preserve your theme's look and functionality. To understand how child themes work it is first important to understand the relationship between parent and child themes.", "kivicare");
						?>
						<a href="<?php echo esc_url($action_url); ?>" target="_blank"><?php echo esc_html($action); ?></a>
					</p>
				</div>
	...
	protected function content()
	{
		...
		?>
			<div class="plugin-installation-error">
				...
				<h1>
					<?php esc_html_e("Something went worng !", "kivicare"); ?>
				</h1>
				<strong>
					<?php esc_html_e(" Below listed plugin has been not installed. Install it mannually or submit the ticket for Help.", "kivicare"); ?>
				</strong>
				<ul class="merlin__drawer--install-plugins">
					<?php foreach ($plugins['all'] as $slug => $plugin) { ?>
						<li>
							...
							<span class="download-link">
								<a href="<?php echo esc_url($url) ?>" target="_blank"><?php esc_html_e("Download", "kivicare"); ?></a>
							</span>
						</li>
					<?php } ?>
				</ul>
				<div class="plugin-installation-links">
					<a href='<?php echo admin_url('themes.php?page=kivicare-setup&step=plugins'); ?>' class='merlin__button merlin__button--knockout merlin__button--no-chevron merlin__button--external'><?php esc_html_e("Try Again", "kivicare"); ?></a>
					<a href="https://iqonic.desky.support/" class="merlin__button merlin__button--knockout merlin__button--no-chevron merlin__button--external" target="_blank"><?php esc_html_e('Get Support', 'kivicare'); ?></a>
				</div>
			</div>
		<?php
			return;
	...
	public function generate_child_functions_php($slug)
	{
		...
		$output = "
			 ...
			 * This child theme was generated by Kivicare.
			 *
...
<?php
	...
	public function kivicare_download_file()
	{
		if (isset($_GET["download_item"]) && !empty($_GET['download_item']) && is_user_logged_in()) {
			$this->kivicare_send_download_file($_GET['download_item']);
		}
	}
	public	function kivicare_send_download_file($remoteURL)
	{

		header("Content-type: application/x-file-to-save");
		header("Content-Disposition: attachment; filename=" . basename("https://assets.iqonic.design/wp/plugins/streamit-new/" . $remoteURL . ".zip"));
		while (ob_get_level()) {
			ob_end_clean();
		}
		readfile($remoteURL);
