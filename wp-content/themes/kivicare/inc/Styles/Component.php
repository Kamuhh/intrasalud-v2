<?php
/**
 * Kivicare\Utility\Styles\Component class
 *
 * @package kivicare
 */

namespace Kivicare\Utility\Styles;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	public function initialize() {
		...
		add_filter( 'wp_resource_hints', array( $this, 'filter_resource_hints' ), 10 , 10 );
	}
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'kivicare-fonts', $google_fonts_url, array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = kivicare()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'kivicare-fonts', 'queue' ) ) {
			$urls[] = array(
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'kivicare' ), $handle ) ), 'Kivicare 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'kivicare_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! kivicare()->is_amp();
		 ...
		 */
		return apply_filters( 'kivicare_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = array(
			...
			'font-awesome-all-min'			=> array(
				'file'   => 'vendor/font-awesome/css/all.min.css',
				'global' => true,
			),
			'ionicons.min'			=> array(
				'file'   => 'vendor/ionicons.min.css',
				'global' => true,
			),
			'wow'			=> array(
				'file'   => 'vendor/wow.css',
			...
			'kivicare-global'     => array(
				'file'   => 'global.min.css',
				'global' => true,
			),
			'kivicare-dummy'      => array(
				'file'   => 'dummy.min.css',
				'preload_callback' => '__return_true',
				'global' => true,
			),
			'kivicare-button'     => array(
				'file'   => 'button.min.css',
				'global' => true,
			),
			'kivicare-comments'   => array(
				'file'             => 'comments.min.css',
				'preload_callback' => function() {
					return ! post_password_required() && is_singular() && ( comments_open() || get_comments_number() );
				},
			),
			'kivicare-woocommerce'     => array(
				'file'   => 'woocommerce.min.css',
				'global' => true,
			),
			'kivicare-sidebar'    => array(
				'file'             => 'sidebar.min.css',
				'preload_callback' => function() {
					return kivicare()->is_primary_sidebar_active();
				},
				'global' => true,
			),
			'kivicare-widgets'    => array(
		 ...
		 */
		$css_files = apply_filters( 'kivicare_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		...
		$google_fonts = array(
			'Roboto' => array( '300' ,'300i', '400','400i' ,'500', '500i','700', '700i' ),
			'Heebo'     => array( '300', '400', '400i','500', '500i', '600', '600i', '700', '700i','800', '800i','900', '900i'),
		);
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'kivicare_google_fonts', $google_fonts );
