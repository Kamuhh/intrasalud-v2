...
>
	...
	if ( ! empty( $page_title ) ) :
		...
		<div class="wishlist-title-container">
			<div class="wishlist-title <?php  $can_user_edit_title  ? esc_attr_e( 'wishlist-title-with-form','kivicare') : ''; ?>">
				<?php echo apply_filters( 'yith_wcwl_wishlist_title', '<h3>' . $page_title . '</h3>' ) ; ?>
				<?php if ( $can_user_edit_title ) : ?>
					<a class="button show-title-form">
						<?php echo yith_wcwl_kses_icon( apply_filters( 'yith_wcwl_edit_title_icon', '<i class="fas fa-edit"></i>' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
						<?php esc_html_e( 'Edit title', 'kivicare' ); ?>
					</a>
				<?php endif; ?>
			</div>
			<?php if ( $can_user_edit_title ) : ?>
