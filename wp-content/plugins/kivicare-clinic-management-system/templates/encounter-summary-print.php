 ...
 * Usa variables ya preparadas por el controlador:
...
<body>
  ...
  <div class="card">
    ...
    <?php if (!empty($diagnoses)) : ?>
            <ul><?php foreach ($diagnoses as $d): ?><li><?php echo $fmt($d['title'] ?? ''); ?></li><?php endforeach; ?></ul>
    <?php else: ?><div class="muted">No se encontraron registros</div><?php endif; ?>
  </div>

  <div class="card">
    <h3>Indicaciones</h3>
    <?php if (!empty($indications)) : ?>
      <ul><?php foreach ($indications as $i): ?><li><?php echo $fmt($i['title'] ?? ''); ?></li><?php endforeach; ?></ul>
    <?php else: ?><div class="muted">No se encontraron registros</div><?php endif; ?>
  </div>
  ...
  <div class="card">
    ...
    <?php else: ?><div class="muted">No se encontró receta</div><?php endif; ?>
  </div>

  <!-- Página 2: Órdenes con firma del médico -->
    ...
    <?php if (!empty($orders)) : ?>
      <ul>
        <?php foreach ($orders as $o): ?>
          <li><?php echo $fmt($o['title'] ?? ''); ?><?php if (!empty($o['note'])): ?> — <span class="muted"><?php echo $fmt($o['note']); ?></span><?php endif; ?></li>
        <?php endforeach; ?>
      </ul>
    <?php else: ?><div class="muted">No se encontraron registros</div><?php endif; ?>
  </div>
  ...
  <div class="doctor-footer">
    <?php if ($signature_url): ?><img class="sig" src="<?php echo esc_url($signature_url); ?>" alt="firma"><?php endif; ?>
    <div class="name"><?php echo $fmt($doc_name); ?></div>
    <?php if ($doc_specialty): ?><div class="line"><?php echo $fmt($doc_specialty); ?></div><?php endif; ?>
    <div class="line">
      <?php
        $cred = [];
