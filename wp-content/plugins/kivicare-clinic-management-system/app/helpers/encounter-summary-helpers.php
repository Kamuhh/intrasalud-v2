...
if (!function_exists('kc_build_encounter_summary_payload')) {
    /**
     * Construye todos los datos necesarios para el PDF del resumen.
     * Reúne datos de paciente, doctor, clínica y las listas de diagnósticos,
     * indicaciones, órdenes clínicas y recetas, así como la firma y credenciales del médico.
     */
    function kc_build_encounter_summary_payload($encounter_id) {
        $encounter_id = (int)$encounter_id;

        // Consulta básica del encuentro
        $e = kc_get_encounter_by_id($encounter_id);
        ...
        // Listas de diagnósticos, indicaciones, órdenes y recetas
        $diagnoses = kc_get_encounter_problems($encounter_id);
        $orders    = kc_get_encounter_orders($encounter_id);
        $indics    = kc_get_encounter_indications($encounter_id);
        $rx        = kc_get_encounter_prescriptions($encounter_id);

        // Logo de la clínica (si tiene imagen subida) y dirección compacta
        $logo = '';
        if (!empty($clinic['profile_image'])) {
            $url = wp_get_attachment_url($clinic['profile_image']);
            if ($url) $logo = $url;
        }
        if (!$logo && defined('KIVI_CARE_DIR_URI')) {
            $logo = KIVI_CARE_DIR_URI . 'assets/images/kc-demo-img.png';
        }
        // Generamos la dirección compacta
        $addr_parts = [];
        ...
        // Especialidad y documento de identidad del doctor
        ...
        // MPPS y CM (campos personalizados 7 y 8)
